...
Singleton {
    ...
    property color color2: "#37B679" 
    property color color3: "#FF9F00"  
    ...
    FileView {
        ...
        function getHomeDir() : string {
            // Use QtResolve to the the abosule path of shell.qml
            // then use regex to get the absolute path of the home directory (and the user name)
            // to achieve dynamic pathing that is not swallowed by resolvedUrl blackhole
            // when trying to navigate outside of the Qml folder boundaries
            var path = Qt.resolvedUrl("./").toString().match(new RegExp("\/home\/(.*?\/){1}"))[0];
            return path;
        }
        id: pywal
        path: getHomeDir() + ".cache/wal/colors.json"
        watchChanges:true
        onLoaded: root.loadFromJson(pywal.text())
        onFileChanged: {
            reload()
            root.loadFromJson(pywal.text())
        }
    }
}
